...
function setupIdleCardAnimations() {
  ...
  document.querySelectorAll('.board-matrix .card[data-affordable="true"]').forEach((el, i) => {
    ...
    // 🚀 第一處修改：調整 Tween 是否重建判定條件，移除已死或被外部變更過濾的 Tween 控制
    if (window._idleTweensMap.has(cardId)) {
      ...
      if (existingTween && !existingTween.killed) {
        return; // 只要 tween 未被 kill 就保留，不用 isActive()（repeat:-1 折返時會誤判）
      }
      window._idleTweensMap.delete(cardId); // tween 已死，清除後重建
    }
    
    // 🚀 第二處修改：加載 gsap.killTweensOf 防止重複選取同色 token 刷新導致多重動畫堆疊
