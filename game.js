...
function renderDashboardGems(targetElementId, actorData, diffs) {
  ...
  // 🚀 第一段修改：重繪前改為保存真實節點參考，先從 DOM 移除，避免被 innerHTML 毀滅同時中斷動畫時間軸
  ...
  container.querySelectorAll('.floating-diff').forEach(span => {
    ...
    if (block && block.id) {
      ...
      span.remove(); // 從原位置分離（Detach），保持 animation timeline 不中斷
      survivingDiffs[color].push(span); // 直接存放真實節點參考
    }
  });
  ...
  // 🚀 第二段：將尚未播完的真實舊節點原封不動移動（Move）插回，動畫將完美延續播放
