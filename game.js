...
// 🚀 Bug 2 修正點：已成功刪除原本第 32-33 行的 let selectedDiff 與 let selectedSame 局部變數宣告，改為全面對接 CoreState 全域狀態
...
window.render = function() {
  ...
  if (sameLayer) {
    sameLayer.innerHTML = gemColors.map(k => {
      const canTake2 = fullState.bank[k] >= 2; 
  ...
  // 🚀 Bug 1 修正點：在 setupIdleCardAnimations 上方，精準插入對確認收下按鈕狀態的 disabled 更新控制
...
// 5. 其餘事件分發代理
// ==========================================
// 🚀 Bug 2 修正點：重構以下四個操作函式，全面改為透過 CoreState.get() 讀取與修改全域選取屬性
