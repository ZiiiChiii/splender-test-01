...
window.render = function() {
  ...
  const diffLayer = document.getElementById('diff-selectors'); // 配合 index.html 內的真實 id
  const sameLayer = document.getElementById('same-selectors'); // 配合 index.html 內的真實 id
  ...
  if (sameLayer) {
    sameLayer.innerHTML = gemColors.map(k => {
      const canTake2 = fullState.bank[k] >= 2; // 回歸原始規則
