...
// 🚀 核心修復：加入動畫生存鎖，重繪時保留並還原尚未播完的舊 span 節點
function renderDashboardGems(targetElementId, actorData, diffs) {
  const container = document.getElementById(targetElementId);
  if (!container) return;

  // 【關鍵】重繪前先把還在播放 floatUpFade 動畫的舊 floating-diff span 快照暫存起來
  ...
  container.querySelectorAll('.floating-diff').forEach(span => {
    ...
    if (block && block.id) {
      ...
      survivingDiffs[color].push(span.cloneNode(true));
    }
  });
  ...
  // 【關鍵】重繪後，將那些還沒播放完毕的舊 span 節點插回對應的金庫中，防止被 innerHTML 吞掉
...
window.render = function() {
  ...
  // 立即更新基準值快照（解除 isAnimating 鎖定與依賴，防重播）
