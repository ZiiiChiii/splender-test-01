...
function animateCardFlightToGoldVault(cardId, providesColor, callback) {
  ...
  .to(flyCard, {
    ...
    onComplete: () => {
      ...
      // 🚀 第四步修正：移除原本在此更新 lastPlayerState 與 render() 的呼叫，防止多餘動畫重播
...
// 🚀 第一步修正：renderDashboardGems 改為純靜態 innerHTML + 命令式注入追加動畫效果
function renderDashboardGems(targetElementId, actorData, diffs) {
  ...
  // 動畫注入：在 DOM 重寫後立即追加 floating-diff 與 pulse 發光效果（不依賴 innerHTML，獨立生命週期）
  if (diffs) {
    ['w', 'u', 'g', 'r', 'k', 'o'].forEach(k => {
      ...
      // 追加 floating-diff span（直接 appendChild）
      ...
      // 動畫結束後精準移除節點（floatUpFade 持續 1.2s）
...
window.render = function() {
  ...
  // 🚀 第二步修正：將快照計算與 lastPlayerState 的更新提前到 renderDashboardGems 呼叫前
  ...
  // 🚀 第三步修正：已完全移除原本舊位置的「if (!isAnimating) { lastPlayerState = deepClone(player); }」
