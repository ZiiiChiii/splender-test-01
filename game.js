...
window.render = function() {
  ...
  ['lv1', 'lv2', 'lv3'].forEach(level => {
    ...
    document.getElementById(`row-${level}`).innerHTML = fullState.board[level].map((card, idx) => {
      ...
      // 🚀 第一步修正：卡牌渲染時，將買得起的布林狀態寫入 data-affordable 屬性中
      return `
...
// 🚀 第二步修正：重構浮空控管，只對 data-affordable="true" 建立補間，並精準重置「變為買不起」的卡牌
function setupIdleCardAnimations() {
  const affordableCardIds = new Set();
  ...
  // 清除所有「不再買得起」或「已離場」的卡牌補間，並進行歸位重置
